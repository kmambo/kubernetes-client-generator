...
def main():
    ...
    if os.environ.get("OPENAPI_SKIP_FETCH_SPEC") or False:
        ...
        spec_filepath = 'https://raw.githubusercontent.com/%s/%s/' \
                        '%s/api/openapi-spec/swagger.json' % (args.username,
                                                              args.repository,
                                                              args.kubernetes_branch)
        with pool.request('GET', spec_filepath, preload_content=False) as response:
            if response.status != 200:
                print("Error downloading spec file %s. Reason: %s" % (spec_filepath, response.reason))
                return 1
